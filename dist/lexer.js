...
/**
 * Lexer for tokenizing SPL queries
 */
class Lexer {
    constructor(_input) {
        this._input = _input;
    ...
    nextToken() {
        ...
        throw new exceptions_1.SPLError(`Invalid character ${this.currentChar} at position ${this.currentPosition}`);
    }
    _skipWhitespace() {
    ...
    _readTo(endChar) {
        ...
        while (this.currentChar !== endChar) {
            if (!this.currentChar) {
                throw new exceptions_1.SPLError("Unterminated string");
            }
            result += this.currentChar;
            this._position++;
        }
